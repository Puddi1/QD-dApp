...
if (!window.CSS.supports("backdrop-filter", "blur(10px)")) {
  sidebar.style.backgroundImage =
    "linear-gradient(rgb(75, 0, 0, 0.9), rgba(50, 0, 0, 0.5))";
}
...
// Create contracts
let _AvalancheValidatorFacet = new ethers.Contract(
  diamondAddress,
  AvalancheValidatorFacet.abi,
  provider
);
let _ERC1155Facet = new ethers.Contract(
  diamondAddress,
  ERC1155Facet.abi,
  provider
);
let _AvalancheValidatorSettersAndGettersFacet = new ethers.Contract(
  diamondAddress,
  AvalancheValidatorSettersAndGettersFacet.abi,
  provider
);
let _AvalancheValidatorDepositFacet = new ethers.Contract(
  diamondAddress,
  AvalancheValidatorDepositFacet.abi,
  provider
);
let _AvalancheValidatorHealthAndUpgradesFacet = new ethers.Contract(
  diamondAddress,
  AvalancheValidatorHealthAndUpgradesFacet.abi,
  provider
);
let _AvalancheValidatorTokenFacet = new ethers.Contract(
  diamondAddress,
  AvalancheValidatorTokenFacet.abi,
  provider
);
let _AvalancheValidatorViewTokenFacet = new ethers.Contract(
  diamondAddress,
  AvalancheValidatorViewTokenFacet.abi,
  provider
);
...
metamaskButton.addEventListener("click", async () => {
  ...
  if (typeof window.ethereum != "undefined") {
    ...
    signerAvalancheBalance = Number(
      (await provider.getBalance(address)).toString()
    );
    shareCost = Number(
      (
        await _AvalancheValidatorSettersAndGettersFacet.getShareCost()
      ).toString()
    );
    signerThorBalance = Number(
      (await _thorIERC20.balanceOf(address)).toString()
    );
    thorPrice = Number(
      (
        await _AvalancheValidatorViewTokenFacet.fetchTokenPrice(thorToken)
      ).toString()
    );
    odinPriceNFT = Number(
      (
        await _AvalancheValidatorViewTokenFacet.fetchTokenPrice(odinNFT)
      ).toString()
    );
    odinPriceNFT = 0.24 * 1e18;
    thorPriceNFT = Number(
      (
        await _AvalancheValidatorViewTokenFacet.fetchTokenPrice(thorNFT)
      ).toString()
    );
    thorPriceNFT = 0.02 * 1e18;

    // connect user to contracts
    signer_AvalancheValidatorDepositFacet =
      _AvalancheValidatorDepositFacet.connect(signer);
    signer_AvalancheValidatorFacet = _AvalancheValidatorFacet.connect(signer);
    signer_AvalancheValidatorHealthAndUpgradesFacet =
      _AvalancheValidatorHealthAndUpgradesFacet.connect(signer);
    signer_AvalancheValidatorTokenFacet =
      _AvalancheValidatorTokenFacet.connect(signer);
...
let buttonToken_Companions = document.getElementById(
  "companions-suggestion-div"
);
...
async function whiteAvaxLogoUpdate() {
  let totalSupply = (
    await _AvalancheValidatorSettersAndGettersFacet.getTotalCurrentSupplyPondered()
  ).toString();
...
let dropdownTokenContainer = document.getElementById(
  "dropdown-token-container"
);
let buttonClosingToken = document.getElementById(
  "dropdown-token-header-button"
);
...
let dropdownTokenContainer_Companions = document.getElementById(
  "dropdown-token-container-companions"
);
let buttonClosingToken_Companions = document.getElementById(
  "dropdown-token-header-button-companions"
);
...
let dropdownSection_Companions = document.getElementById(
  "dropdown-token-section-companions"
);
window.addEventListener("click", (e) => {
  if (
    !isElementOnPath(e, dropdownSection) &&
    dropdownTokenContainer.style.display == "flex" &&
    !isElementOnPath(e, buttonToken)
  ) {
    dropdownTokenContainer.style.display = "none";
  }
  if (
    !isElementOnPath(e, dropdownSection_Companions) &&
    dropdownTokenContainer_Companions.style.display == "flex" &&
    !isElementOnPath(e, buttonToken_Companions)
  ) {
    dropdownTokenContainer_Companions.style.display = "none";
  }
});
...
async function updateCompanion() {
  let companion = (
    await _AvalancheValidatorSettersAndGettersFacet.getReferredAddress(
...
addReferralButton.addEventListener("click", async () => {
  ...
  let correctResponse =
    inputAddressCompanion.value[0] == "0" &&
    inputAddressCompanion.value[1] == "x" &&
    inputAddressCompanion.value.length == 42;
  if (correctResponse) {
    try {
      let tx =
        await signer_AvalancheValidatorHealthAndUpgradesFacet.manageReferralAddress(
          inputAddressCompanion.value
        );
      await tx.wait(1);
...
let avaxIcon =
  '<img id="avalanche-icon-selected" src="../miniEmptyWhiteAvaxLogo.svg" alt="Token">';
let avaxName = "AVAX";
...
let OGOdinIcon =
  '<img id="OGOdin-icon-selected" src="../OGOdinWhite.svg" alt="Token">';
let OGOdinName = "OG ODIN";
let OGThorTokenIcon = document.getElementById("OGthor-token");
let OGThorIcon =
  '<img id="OGThor-icon-selected" src="../OGThorWhite.svg" alt="Token">';
...
// Left / remaining
let remainingContainer = document.getElementById(
  "mintingInput-remaining-container"
);
let remainingLeftNumber = document.getElementById("left-leftNumber-paragraph");
let remainingRightNumber = document.getElementById(
  "left-rightNumber-paragraph"
);
...
avaxTokenIcon.addEventListener("click", async () => {
  // Select shares
  inputToken.value = "";
  inputShares.value = "";
...
thorTokenIcon.addEventListener("click", async () => {
  // Select shares
  displayTokenGrid.innerHTML = "";
  inputToken.value = "";
  inputShares.value = "";
  ...
  let thorTokenAvaxDeposit = Number(
    (
      await _AvalancheValidatorViewTokenFacet.fetchTokenPrice(thorToken)
    ).toString()
  );
  let maxDepositShare = Math.floor(thorTokenAvaxDeposit / shareCost);
  let maxThorTokenShares = Math.floor(
    signerThorBalance / (shareCost / thorPrice)
  );
  baseAvaxRemaining =
    maxDepositShare > maxThorTokenShares ? maxThorTokenShares : maxDepositShare;
  remainingRightNumber.innerHTML = baseAvaxRemaining;
  remainingRightNumber.style.opacity = "1";
...
OGOdinTokenIcon.addEventListener("click", async () => {
  // Select tokens
  inputToken.value = "";
  inputShares.value = "";
  ...
  let maxAvaxDepositNFTs = Math.floor(
    thorTokenAvaxDeposit / (odinPriceNFT * 1e18)
  );
  baseAvaxRemaining =
    maxAvaxDepositNFTs > nftOwner ? nftOwner : maxAvaxDepositNFTs;
...
OGThorTokenIcon.addEventListener("click", async () => {
  // Select tokens
  inputToken.value = "";
  inputShares.value = "";
  ...
  let maxAvaxDepositNFTs = Math.floor(
    thorTokenAvaxDeposit / (thorPriceNFT * 1e18)
  );
  baseAvaxRemaining =
    maxAvaxDepositNFTs > nftOwner ? nftOwner : maxAvaxDepositNFTs;
  remainingLeftNumber.innerHTML = baseAvaxRemaining;
...
inputShares.addEventListener("input", async () => {
  if (nameButton.innerHTML == avaxName && inputShares.value != "") {
    // calculate value := shares * shareCost
    if (shareCost * Number(inputShares.value) <= signerAvalancheBalance) {
      inputToken.value = "" + (shareCost * Number(inputShares.value)) / 1e18;
      remainingRightNumber.innerHTML =
        Number(baseAvaxRemaining) - Number(inputShares.value);
    } else {
      inputShares.value = "";
    }
  } else if (nameButton.innerHTML == thorName && inputShares.value != "") {
    // calculate value := shares * shareCost * price, where price = THOR / AVAX
    // add liquidity as if condition
    if (
      (shareCost * Number(inputShares.value)) / thorPrice <=
      signerThorBalance
    ) {
      inputToken.value =
        "" + (shareCost * Number(inputShares.value) * thorPrice) / 1e18;
      remainingRightNumber.innerHTML =
        Number(baseAvaxRemaining) - Number(inputShares.value);
...
inputToken.addEventListener("input", async () => {
  ...
  if (
    nameButton.innerHTML == OGOdinName &&
    inputToken.value <= Number(baseAvaxRemaining) &&
    inputToken.value != ""
  ) {
    // calculate amount := tokens * price / shareCost, where price = AVAX / OGODIN
    // add liquidity as if condition
    ...
    if (inputToken.value <= NFTs.length) {
      ...
      inputShares.value =
        Math.floor(odinPriceNFT / shareCost) * inputToken.value;
      remainingLeftNumber.innerHTML =
        Number(baseAvaxRemaining) - Number(inputToken.value);
    }
  } else if (
    nameButton.innerHTML == OGThorName &&
    inputToken.value <= Number(baseAvaxRemaining) &&
    ...
    // calculate amount := tokens * price / shareCost, where price = AVAX / OGTHOR
    // add liquidity as if condition
    // Select nfts and related IDs from start to finish
    if (inputToken.value <= NFTs.length) {
      for (let i = 0; i < inputToken.value; i++) {
      ...
      // Calculate shares everytime it changes
      inputShares.value =
        Math.floor(thorPriceNFT / shareCost) * inputToken.value;
      remainingLeftNumber.innerHTML =
        Number(baseAvaxRemaining) - Number(inputToken.value);
    }
  }
...
async function addListener(_elements, priceNFT) {
  // Add array of NFTs which are objects w/ id and bool selected then at the end filter selected and send related to the contract
  for (const e of _elements) {
    e.addEventListener("click", () => {
      ...
      if (
        ...
        Number(inputToken.value) + 1 <= Number(baseAvaxRemaining)
      ) {
        console.log("test");
        ...
        inputShares.value =
          Math.floor(priceNFT / shareCost) * Number(inputToken.value);
        remainingLeftNumber.innerHTML =
          Number(baseAvaxRemaining) - Number(inputToken.value);
...
async function checkApproval721(contract) {
  let approved = (
    await contract.isApprovedForAll(await signer.getAddress(), diamondAddress)
  ).toString();
  if (approved === "false") {
    // Allow
...
async function checkApproval20(contract, amount) {
  let approvedAmount = Number(
    (
      await contract.allowance(await signer.getAddress(), diamondAddress)
    ).toString()
  );
  if (approvedAmount == 0 || approvedAmount < amount) {
    // Allow
    buttonInput.innerHTML = "ALLOW";
  } else {
    // Active
...
async function addOGThorNFTs() {
  ...
  xhr.open(
    "GET",
    baseURL +
      "/Moralis/WalletNFTs/AVALANCHE/0x0658B45130B69aaD47A01a3a898bE5EEDEcb24C3/0x825189515d0A7756436F0eFb6e4bE5A5aF87e21D",
    false
  );
  ...
  if (xhr.readyState === 4 && xhr.status === 200) {
    ...
    await response.forEach((e) => {
      ...
      let display =
        "<div class='token-grid' nonce='" +
        e.token_id +
        "'><div class='token-image'><img src='" +
        OGThorImage +
        "' alt='Image token'></div><div class='token-label'><p class='token-name'><strong>" +
        e.metadata.split('"')[3] +
        "</strong></p><p><strong>" +
        e.token_id +
        "</strong></p></div></div>";
...
async function addOGOdinNFTs() {
  ...
  xhr.open(
    "GET",
    baseURL +
      "/Moralis/WalletNFTs/AVALANCHE/0x0658B45130B69aaD47A01a3a898bE5EEDEcb24C3/0x7325e3564b89968d102b3261189ea44c0f5f1a8e",
    false
  );
  // send the Http request
  xhr.send(null);
...
buttonInput.addEventListener("click", async () => {
  ...
  if (buttonInput.innerHTML == "ALLOW") {
    if (nameButton.innerHTML == thorName) {
      try {
        let tx = await signer_thorIERC20.approve(
          diamondAddress,
          ethers.utils.parseEther(inputToken.value)
        );
        let recepit = await tx.wait(1);
      ...
      await checkApproval20(
        _thorIERC20,
        ethers.utils.parseEther(inputToken.placeholder)
      );
    } else if (nameButton.innerHTML == OGThorName) {
      try {
        let tx = await signer_thorIERC721.setApprovalForAll(
          diamondAddress,
          true
        );
        let recepit = await tx.wait(1);
      ...
      try {
        let tx = await signer_odinIERC721.setApprovalForAll(
          diamondAddress,
          true
        );
        let recepit = await tx.wait(1);
    ...
    if (nameButton.innerHTML == avaxName) {
      ...
      if (0 < inputShares.value) {
        ...
        let _value = ethers.utils.parseEther(
          "" + (inputShares.value * _shareCost).toFixed(10)
        );
        console.log("test");
        try {
          let txnMint =
            await signer_AvalancheValidatorDepositFacet.mintAvalancheValidatorShareAVAX(
              inputShares.value,
              "0x",
              { value: _value, gasLimit: 1_000_000 }
            );
          let recepit = await txnMint.wait(1);

          messageHeader.innerHTML = "Succesfull Mint";
          messageBody.innerHTML =
            "Transaction hash: " + recepit.transactionHash;
          setTimeout(() => {
            messageContainer.classList.add("visibleMessage");
          }, 500);
        } catch (err) {
      ...
      if (0 < inputShares.value) {
        try {
          let txnMint =
            await signer_AvalancheValidatorTokenFacet.mintAvalancheValidatorShareTokenERC20(
              thorToken,
              inputShares.value,
              { gasLimit: 1_000_000 }
            );
      ...
      if (IDs.length > 0) {
        try {
          let txnMint =
            await signer_AvalancheValidatorTokenFacet.mintAvalancheValidatorShareTokenERC721(
              odinNFT,
              IDs,
              { gasLimit: 1_000_000 }
            );
...
async function displaySupply() {
  ...
  for (let i = 0; i < 5; ++i) {
    balances[i] = (
      await _ERC1155Facet.balanceOf(await signer.getAddress(), i)
    ).toString();
  }
...
let strokeVisualizers1 = document.getElementsByClassName(
  "stroke-lifetimeVisualizer-level1"
);
let strokeVisualizers2 = document.getElementsByClassName(
  "stroke-lifetimeVisualizer-level2"
);
let strokeVisualizers3 = document.getElementsByClassName(
  "stroke-lifetimeVisualizer-level3"
);
let strokeVisualizers4 = document.getElementsByClassName(
  "stroke-lifetimeVisualizer-level4"
);
let strokeVisualizers5 = document.getElementsByClassName(
  "stroke-lifetimeVisualizer-level5"
);
async function displayHealth() {
  console.log("Refreshing health");
  let health = [];
  let timestamp = (await provider.getBlock(await provider.getBlockNumber()))
    .timestamp;

  for (let i = 0; i < 5; ++i) {
    let ownerHealth = (
      await _AvalancheValidatorSettersAndGettersFacet.getOwnerHealth(
        await signer.getAddress(),
        i
      )
    ).toString();
    ...
    if (ownerHealth > 0 && ownerHealth <= timestamp) {
      let activityPeriod = (
        await _AvalancheValidatorSettersAndGettersFacet.getActivityPeriod()
      ).toString();
      health[i] = Math.round(
        (activityPeriod - (timestamp - ownerHealth)) / 3600
      ); // 86400 for 31d for 12h you eed hours: 60*60
...
async function displayRewards() {
  ...
  rewards = await _AvalancheValidatorSettersAndGettersFacet.getAllOwnerRewards(
    await signer.getAddress()
  );
...
rewardIcon.addEventListener("click", async () => {
  ...
  if (!(rewardIDs == 0)) {
    try {
      let tx = await signer_AvalancheValidatorFacet.collectRewards(rewardIDs, {
        gasLimit: 1_000_000,
      });
      await tx.wait(1);
      await displayRewards();
...
healthIcon.addEventListener("click", async () => {
  ...
  if (!(healthIDs == 0)) {
    try {
      let tx =
        await signer_AvalancheValidatorHealthAndUpgradesFacet.refreshAvalancheValidatorSharesHealth(
          healthIDs,
          { gasLimit: 1_000_000 }
        );
      await tx.wait(1);
...
async function updateChanges(_address, _networkId) {
  ...
  if (
    ...
    (_networkId != "0xa869" || _networkId != 43113) &&
    _networkId != undefined // testnet
  ) {
